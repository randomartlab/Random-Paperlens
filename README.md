...
### 范式拆解：按学科范式而非固定模板（请谨慎采信）

这是本项目的核心之一。**不同的研究范式，评审标准与信息结构本就不同** —— 用一套固定字段去拆解所有论文，得到的只能是泛泛而谈。

应用内建八种研究范式的独立字段库，先识别论文属于哪种范式，再据此组装拆解字段（通用字段 + 范式特有字段，交叉学科则合并多套字段），最后逐字段执行拆解。

识别过程是透明的：主范式、置信度、识别依据（命中了哪些章节信号与关键词）、交叉构成，全部展示出来。

**完整链路**——拆解时这几步都是真实执行的，不是配置项：

```
PDF
 └─ 版面解析 ──→ Markdown（保留标题层级 / 表格 / 公式 / 图片）
      └─ 信号提取 ──→ 标题、摘要、章节标题、关键词、表格数、公式数
           └─ 范式识别 ──→ 主范式 / 次范式 / 置信度 / 交叉类型
                └─ 字段路由 ──→ 通用字段 ∪ 范式特有字段（∪ 交叉对话字段）
                     └─ 逐字段拆解 ──→ 每字段一次请求，结论锚定原文段落
```

范式识别与字段路由是**拆解任务内部现算的**——前端只传文献 ID，不存在"界面显示一套范式、实际按另一套拆解"的脱节。识别结果同时写进拆解提示词，所以不同范式的核对侧重也不同（实验对比型核对基线是否公平，综述型核对检索策略是否可复现）。
...
解析结果直接在应用内阅读，公式、表格、图片正常渲染。

**右键摘录**：鼠标停在某句上直接右键，摘的就是**这一整句**（双语视图下连对应译文一起带上）；如果想只要片段，**拖选**之后右键，摘的是所选内容。摘录的出处标注为「章节路径 · 第 N 段」（例如 `2. Related studies › 2.1. Body movements · 第 12 段`），而不是重复一遍笔记名里已有的题目——贴进笔记后能直接回答"这段出自论文哪里"。右键菜单里会先预览这次要摘的内容与出处，确认无误再落笔。

**引用可跳转**：正文里的 `[1]`、`[2,3]` 这类引用标记可以直接点击，跳到文末参考文献的对应条目；核对一条引用的出处不再需要手动翻页。

**添加到笔记**：首次添加会按 `月-日-题目-一作-年份-阅读笔记` 自动建立笔记（缺失项自动跳过，题目过长会截断保留一作与年份），之后同一篇文献再添加就追加到同一份笔记末尾，用分隔线隔开。一篇文献的摘录会自然归拢在一处，不必手动整理文件名。

**笔记管理**（顶栏「笔记」页）：笔记以本地 Markdown 文件存放，可以直接在文件系统里查看、编辑、备份。应用内提供列表与编辑、重命名、批量删除、跨笔记查找替换，以及**导出**——单篇或批量导出为 Markdown / HTML（自包含，可离线打开），或走系统打印窗口存为 PDF。

复制与选取是并存的：想精确选某个词、某句话时照常拖选即可，只有**在没有选区的情况下单击**才会触发整句 / 整条复制，不会妨碍精细选取。

### 文献库：双击直达原文

文献卡片单击是常规打开（默认视图），**双击则跳过译文与拆解，直接落到原文视图**——想把一篇论文当普通 PDF 通读时，少两次切换。

卡片上的「未读 / 已读」开关用于标记阅读进度，列表顶部可按状态筛选。

### 任务中心：处理进度看得见

解析、翻译、拆解都是耗时任务。「任务中心」把正在进行与刚完成的文献任务集中列出，带上进度与状态；发起处理后可以离开这一页去做别的，不必守在原地等结果。
...
七套主题：默认、Minimal、Dracula、Blue Topaz、Catppuccin、Twilight、Amber，全部支持浅色与深色。灵感来自 Obsidian 社区皮肤，切换即时生效并自动保存。

其中 **Twilight（深紫暮色 + 银白）与 Amber（暖琥珀 + 深棕）是"带氛围图"的主题**：文献库（列表）页会整页铺一层氛围背景，顶栏与导航栏半透明吃氛围色；点进去的阅读、笔记、设置等栏目**只继承色调，不铺装饰层**——氛围留在门口，不打扰精读。
...
3. 可选：**视觉模型**用于图注与图片内容识别。**若你配置的主模型本身支持视觉**（GLM-4V、GPT-4o、Claude、Qwen-VL 等），会自动复用主模型，不必另外配置；只有主模型是纯文本模型（如 DeepSeek、GLM 文本版）时，才需要在这里单独填一个视觉模型，否则图片识别会被跳过——不影响解析、翻译与拆解主流程。
...
- **以 PDF 为单元的 AI 侧边对话**——在阅读界面里就当前这篇文献直接提问与追问，省去把内容复制到外部对话里的来回搬运
- **Zotero 联动**——与既有文献管理流程打通，复用已有的元数据与分组，减少重复整理

方向会随实际使用调整，不代表承诺的交付时间。
