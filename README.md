...
解析结果直接在应用内阅读，公式、表格、图片正常渲染。选中任意文本后**右键即可「复制为摘录（含来源）」**，或**添加到笔记**。

**引用可跳转**：正文里的 `[1]`、`[2,3]` 这类引用标记可以直接点击，跳到文末参考文献的对应条目；核对一条引用的出处不再需要手动翻页。

**添加到笔记**：首次添加会按 `月-日-题目-一作-年份-阅读笔记` 自动建立笔记（缺失项自动跳过），之后同一篇文献再添加就追加到同一份笔记末尾，用分隔线隔开。一篇文献的摘录会自然归拢在一处，不必手动整理文件名。
